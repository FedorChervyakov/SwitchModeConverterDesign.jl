...
# ╔═╡ 1e8eaecb-5059-4e55-8f00-5d392b84a840
begin
	using PlutoUI, Plots
end
...
md"""
## Ideal Buck
Efficiency of an ideal buck converter is by definition 100%.
## Lossy Buck with Conduction Losses
### Inductor Copper Loss
This loss is cause by non-zero resistance of the inductor winding. It is defined as:\
$P_{loss\ R_L} = I^2R_L$\
The efficiency of a buck converter including inductor copper loss depends on ratio of inductor resistance to load resistance:\
$\eta = \frac{1}{1 + \frac{R_L}{R}}$
"""

# ╔═╡ 47baaeda-a069-4c05-85ac-1de8de3596c2
nu_RL(D, RL, R) = 1 / (1+RL/R)
...
#### Loss due to MOSFET's ON resistance
Since MOSFET conducts only during DT, loss due to it's ON resistance is proportional to D. Efficiency of a buck converter including only loss due to MOSFET's ON resistance is:\
``\eta = \frac{1}{1 + \frac{R_{ON}}{R} D}``
"""
...
Since diode conducts only during D'T, losses due to it's ON resistance and forward voltage are proportional to D`. Efficiency of a buck converter including only losses due to diodes's ON resistance and forward voltage drop is:\
``\eta = \frac{1 - \frac{V_D D'}{V_g D}}{1 + \frac{R_D}{R} D'}``
"""

# ╔═╡ 2183f050-ed07-41ed-b6c1-f0f3c0a20c77
function nu_diode(D, VD, Vg, RD, R)
	D′ = 1 - D
	return (1 - VD/Vg*D′/D) / (1 + RD/R*D′)
end
...
begin
	nu_diode_plt = plot(
		D, nu_diode.(D, Ref(vd_rat), 1, Ref(rd_rat), 1),
		ylim = (0, 1),
		xlabel = "Duty-cycle D",
		ylabel = "Efficiency η",
		title = "efficiency"
	...
	m_diode_plt = plot(
		D, D.*nu_diode.(D, Ref(vd_rat), 1, Ref(rd_rat), 1),
		ylim = (0, 1),
		xlabel = "Duty-cycle D",
		ylabel = "M(D)",
		title = "Conversion ratio"
...
md"""
### All together now
Including conduction losses due to Rd, Vd, Ron, RL
"""
...
begin
	nu_cond_plt = plot(
		D, nu_cond.(D, Ref(Vd), 1, Ref(RL), Ref(RON), Ref(RD), 1),
		ylim = (0, 1),
		xlabel = "Duty-cycle D",
		ylabel = "Efficiency η",
		title = "efficiency",
	...
	m_cond_plt = plot(
		D, D.*nu_cond.(D, Ref(Vd), 1, Ref(RL), Ref(RON), Ref(RD), 1),
		ylim = (0, 1),
		xlabel = "Duty-cycle D",
		ylabel = "M(D)",
		title = "Conversion ratio"
